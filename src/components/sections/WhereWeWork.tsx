...
const locations = [
  { name: "Dete", note: "Our base in Hwange District, Zimbabwe." },
  {
    name: "Surrounding communities",
    note: "Villages, wards and farming households across the Dete area.",
  },
  {
    name: "Hwange National Park boundary",
    note: "Households living directly alongside wildlife.",
  },
  {
    name: "Matabeleland North",
    note: "The province surrounding our base in Dete.",
  },
];

const { latitude: lat, longitude: lon } = site.where;
const bbox = [lon - 0.22, lat - 0.19, lon + 0.22, lat + 0.19]
  .map((n) => n.toFixed(4))
  .join("%2C");
    ...
    <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
      <div>
        ...
        <p className="mt-5 text-lg leading-relaxed text-ink-500">
          Tikobane Trust is based in Dete, in Hwange district, Matabeleland
          North — immediately adjacent to Hwange National Park. That location
          shapes everything we do: our communities live with wildlife every day,
          not at a distance.
        </p>
        <ul className="mt-8 space-y-4">
          {locations.map((l) => (
            <li
              key={l.name}
              className="flex gap-4 rounded-2xl bg-white p-4 shadow-soft ring-1 ring-forest-900/8"
            >
